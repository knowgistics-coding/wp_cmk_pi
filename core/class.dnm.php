...
class DNM {
  ...
  protected function query($section){
    ...
    if(isset($section["thumbnail"])){ $args['meta_query'] = array(array('key' => '_thumbnail_id')); }
  ...
  protected function get_thumbnail($post,$size=null){
    ...
    if(!!$post["thumbnail"]==false){
      ...
      $post["get_thumbnail"] = get_template_directory_uri().'/customs/sakura.jpeg'; // no-image
    }
    return $post;
  }
  ...
  protected function get_post_category_link($id){
    if(get_theme_mod("toggle_category",true)==false){ return array(); }
  ...
  public function slice_text($text,$start=0,$num=64){
    if(mb_strlen($text,'UTF-8')<=$num){ return $text; }
    else{ return iconv_substr($text, $start, $num, "UTF-8")."..."; }
  }
}
  ...
  public function __construct(){
    ...
    if(!empty($sections)){
      foreach($sections as $json_section){
        ...
        } else { echo '<script>console.log("No Section: '.$section["type"].'");</script>'; }
      }
    }
  }
}
